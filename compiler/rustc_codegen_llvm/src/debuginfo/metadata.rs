...
pub fn file_metadata(cx: &CodegenCx<'ll, '_>, source_file: &SourceFile) -> &'ll DIFile {
    ...
    let file_name = Some(source_file.name.to_string_embeddable());
    let directory = if source_file.is_real_file() && !source_file.is_imported() {
        Some(cx.sess().working_dir.most_stable_name().to_string_lossy().to_string())
    } else {
...
pub fn compile_unit_metadata(
    ...
    let work_dir = tcx.sess.working_dir.most_stable_name().to_string_lossy();
    let flags = "\0";
